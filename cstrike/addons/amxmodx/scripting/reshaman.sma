...
new const zclass_model[] = "reshaman" // model
new const zclass_clawmodel[] = "re/v_re_shaman.mdl" // claw model
new const g_vgrenade[] = "models/zombie_plague/re/v_zombibomb-shaman.mdl"
new const zclass_ring_sprite[] = "sprites/shockwave.spr" // ring sprite
new const zclass_screamsounds[][] = { "zombie_plague/re/shaman/siren_scream.wav" } // scream sound
...
// --- config ------------------------ //
new Float:g_sirencooldown = 50.0 // cooldown time
// ----------------------------------- //
...
g_msgBarTime, g_sprRing, g_maxplayers
...
public plugin_init()
{
	...
	cvar_screamdmg = register_cvar("zp_siren_scream_damage", "2.0")
...
{   
    ...
    if(!zp_get_user_zombie(id) || !is_user_alive(id) || zp_get_user_zombie_class(id) != g_iSirenZID)   
...
public roundStart()
{
	...
	{
		i_cooldown_time[i] = floatround(g_sirencooldown)
		g_abil_one_used[i] = 0
		remove_task(i)
	}
}
...
public ShowHUD(id)
{
	if(is_user_alive(id))
	{
		...
		show_hudmessage(id, "%L: %d", id, "BIG_TIME", i_cooldown_time[id])
	}else{
		remove_task(id)
	}
}
...
public fw_PlayerSpawn_Post(id)
{
    ...
    if(zp_get_user_zombie_class(id) == g_iSirenZID && zp_get_user_zombie(id) && !zp_get_user_nemesis(id))
    {
        // Array
        g_bCanDoScreams[id] = true
        
        // Message
        //client_printcolor(id, "/g[ZP]/y %L", id, "CLASS_SHAMAN")
    }
...
public task_do_scream(id)
{
	...
	{				
        ...
        i_cooldown_time[id] = floatround(g_sirencooldown)
...
public task_reload_scream(id)
{
	...
	client_printcolor(id, "/g[ZP]/y %L", id, "CLASS_SHAMAN2")
}
...
public zp_user_infected_post(id, infector)
{
	...
	if(zp_get_user_zombie_class(id) == g_iSirenZID && !zp_get_user_nemesis(id))
	{
		...
		// Message
		new note_cooldown = floatround(g_sirencooldown)
		client_printcolor(id, "/g[ZP]/y %L", id, "CLASS_SHAMAN", note_cooldown)
