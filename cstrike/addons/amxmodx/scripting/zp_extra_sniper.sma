...
#define M400_WEAPONKEY	803
#define SG1_WEAPONKEY	804
...
new const Fire_Sounds[][] = { "weapons/svd-1.wav", "weapons/vsk-1.wav", "weapons/m400-1.wav" }
new const GUNSHOT_DECALS[] = { 41, 42, 43, 44, 45 }
new SVD_V_MODEL[64] = "models/re/v_svd_fix.mdl"
new SVD_P_MODEL[64] = "models/re/p_svd.mdl"
new SVD_W_MODEL[64] = "models/re/w_svd.mdl"
new VSK94_V_MODEL[64] = "models/re/v_vsk94.mdl"
new VSK94_P_MODEL[64] = "models/re/p_vsk94.mdl"
new VSK94_W_MODEL[64] = "models/re/w_vsk94.mdl"
new M400_V_MODEL[64] = "models/re/v_m400.mdl"
new M400_P_MODEL[64] = "models/re/p_m400.mdl"
new M400_W_MODEL[64] = "models/re/w_m400.mdl"
new cvar_dmg_svd, cvar_recoil_svd, cvar_spd_svd, cvar_ammo_svd, g_itemid_svd
new cvar_dmg_vsk94, cvar_recoil_vsk94, cvar_spd_vsk94, cvar_ammo_vsk94, g_itemid_vsk94
new cvar_dmg_m400, cvar_recoil_m400, g_itemid_m400, cvar_ammo_m400
new cvar_clip_svd, cvar_clip_vsk94, cvar_clip_m400
new g_has_svd[33], g_has_vsk94[33], g_has_m400[33]
new g_MaxPlayers, g_orig_event_svd, g_orig_event_sg550, g_orig_event_awp, g_IsInPrimaryAttack, g_clip_ammo[33], g_svd_TmpClip[33], g_vsk_TmpClip[33]
new Float:cl_pushangle[MAX_PLAYERS + 1][3], m_iBlood[2]

public plugin_init()
{
	register_plugin("[ZP] Extra: New Weapons", "1.1", "Crock")
	...
	cvar_dmg_svd = register_cvar("zp_svd_dmg", "2.0")
	cvar_recoil_svd = register_cvar("zp_svd_recoil", "0.8")
	cvar_clip_svd = register_cvar("zp_svd_clip", "35")
	cvar_ammo_svd = register_cvar("zp_svd_ammo", "90")
	cvar_spd_svd = register_cvar("zp_svd_spd", "1.5")
	
	cvar_dmg_vsk94 = register_cvar("zp_vsk94_dmg", "1.1")
	cvar_recoil_vsk94 = register_cvar("zp_vsk94_recoil", "0.8")
	cvar_clip_vsk94 = register_cvar("zp_vsk94_clip", "40")
	cvar_ammo_vsk94 = register_cvar("zp_vsk94_ammo", "90")
	cvar_spd_vsk94 = register_cvar("zp_vsk94_spd", "0.4")
		
	cvar_clip_m400 = register_cvar("zp_m400_clip", "10")
	cvar_ammo_m400 = register_cvar("zp_m400_ammo", "30")
	cvar_dmg_m400 = register_cvar("zp_m400_dmg", "3")
	cvar_recoil_m400 = register_cvar("zp_m400_recoil", "0.8")

	g_itemid_svd = zp_register_extra_item("[SVD]", 4, ZP_TEAM_HUMAN)
	g_itemid_vsk94 = zp_register_extra_item("[VSK94]", 7, ZP_TEAM_HUMAN)
	g_itemid_m400 = zp_register_extra_item("[M400]", 3, ZP_TEAM_HUMAN)	
...
public zp_extra_item_selected(id, itemid)
{
	...
	{	
		...
		if( iWep2 > 0 )
		{
			...
			cs_set_user_bpammo (id, CSW_G3SG1, get_pcvar_num(cvar_ammo_svd))
		}
		g_has_svd[id] = true;
	}
	else if(itemid == g_itemid_vsk94)
	{	
		drop_weapons(id, 1);
	...
	else if(itemid == g_itemid_m400)
	{
		drop_weapons(id, 1);
		new iWep4 = give_item(id,"weapon_awp")
		if( iWep4 > 0 )
		{
			cs_set_weapon_ammo( iWep4, get_pcvar_num(cvar_clip_m400) )
			cs_set_user_bpammo (id, CSW_AWP, get_pcvar_num(cvar_ammo_m400))
		}
...
public fwPrecacheEvent_Post(type, const name[])
{
	...
	else if (equal("events/sg550.sc", name))
	{
		g_orig_event_sg550 = get_orig_retval()
		return FMRES_HANDLED
	}
	else if (equal("events/awp.sc", name))
	{
...
public fw_SetModel(entity, model[])
{
	...
	if(equal(model, "models/w_g3sg1.mdl"))
	{
		...
		iStoredSVDID = find_ent_by_owner(ENG_NULLENT, "weapon_g3sg1", entity)
		...
		if(g_has_svd[iOwner])
		{
			entity_set_int(iStoredSVDID, EV_INT_WEAPONKEY, SVD_WEAPONKEY)
	...
	else if(equal(model, "models/w_sg550.mdl"))
	{
	...
	else if(equal(model, "models/w_awp.mdl"))
	{
		...
		iStoredAWPID = find_ent_by_owner(ENG_NULLENT, "weapon_awp", entity)
		
		if(!is_valid_ent(iStoredAWPID))
			return FMRES_IGNORED;
		
		if(g_has_m400[iOwner])
		{
			entity_set_int(iStoredAWPID, EV_INT_WEAPONKEY, M400_WEAPONKEY)
			
			g_has_m400[iOwner] = false
			
			entity_set_model(entity, M400_W_MODEL)
...
public fw_AWP_AddToPlayer(AWP, id)
{
	if(!is_valid_ent(AWP) || !is_user_connected(id))
		return HAM_IGNORED;
	
	if(entity_get_int(AWP, EV_INT_WEAPONKEY) == M400_WEAPONKEY)
	{
		g_has_m400[id] = true
...
public CurrentWeapon(id)
{
     ...
     if((read_data(2) != CSW_G3SG1 || !g_has_svd[id]) && (read_data(2) != CSW_SG550 || !g_has_vsk94[id]))
          return
     
     static Float:iSpeed
     if(g_has_svd[id])
          iSpeed = get_pcvar_float(cvar_spd_svd)
     else if(g_has_vsk94[id])
...
replace_weapon_models(id, weaponid)
{
	switch (weaponid)
	{
		...
		case CSW_AWP:
		{
			...
			if(g_has_m400[id])
			{
				set_pev(id, pev_viewmodel2, M400_V_MODEL)
				set_pev(id, pev_weaponmodel2, M400_P_MODEL)
			}
		}
	}
}
...
{
	if(!is_user_alive(Player) || (get_user_weapon(Player) != CSW_G3SG1 && get_user_weapon(Player) != CSW_SG550 && get_user_weapon(Player) != CSW_AWP) || 
        (get_user_weapon(Player) == CSW_G3SG1 && !g_has_svd[Player]) || (get_user_weapon(Player) == CSW_SG550 && !g_has_vsk94[Player]) || (get_user_weapon(Player) == CSW_AWP && !g_has_m400[Player]))
		return FMRES_IGNORED
...
public fw_AWP_PrimaryAttack(Weapon)
{
	new Player = get_pdata_cbase(Weapon, 41, 4)
	
	if (!g_has_m400[Player])
		return;
...
{
	if ((eventid != g_orig_event_svd && eventid != g_orig_event_sg550 && eventid != g_orig_event_awp) || !g_IsInPrimaryAttack)
		return FMRES_IGNORED
...
public fw_AWP_PrimaryAttack_Post(Weapon)
{
	g_IsInPrimaryAttack = 0
	new Player = get_pdata_cbase(Weapon, 41, 4)
...
public SVD__ItemPostFrame(weapon_entity) {
	...
	{
		new j = min(get_pcvar_num(cvar_clip_svd) - iClip, iBpAmmo)
...
public SVD__Reload(weapon_entity) {
	new id = pev(weapon_entity, pev_owner)
	if (!is_user_connected(id))
		return HAM_IGNORED;
	...
	if (iClip >= get_pcvar_num(cvar_clip_svd))
		return HAM_SUPERCEDE;
...
public SVD__Reload_Post(weapon_entity) {
	new id = pev(weapon_entity, pev_owner)
	if (!is_user_connected(id))
		return HAM_IGNORED;
...
public fw_TakeDamage(victim, inflictor, attacker, Float:damage)
{
	if (victim != attacker && is_user_connected(attacker))
	{
		...
		else if(get_user_weapon(attacker) == CSW_SG550)
		{
		...
		{
			if(g_has_m400[attacker])
				SetHamParamFloat(4, damage * get_pcvar_float(cvar_dmg_m400))
		}
	}
}
...
public message_DeathMsg(msg_id, msg_dest, id)
{
	...
	else if(equal(szTruncatedWeapon, "sg550") && get_user_weapon(iAttacker) == CSW_SG550)
	{
		if(g_has_vsk94[iAttacker])
			set_msg_arg_string(4, "vsk94")
	}
	else if(equal(szTruncatedWeapon, "awp") && get_user_weapon(iAttacker) == CSW_AWP)
	{
		if(g_has_m400[iAttacker])
