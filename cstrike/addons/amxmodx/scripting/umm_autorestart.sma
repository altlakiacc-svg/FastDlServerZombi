...
*
*	Переменные поместить в server.cfg:
*		umm_autorestart <0|1|2>			(default: 0)	- Режимы авторестартов: OFF, TIME, ONE ROUND
*															TIME:		авторестарт по истечении заданного интервала времени от 10 до 90 секунд
*															ONE ROUND:	авторестарт после окончания 1-го раунда
*		umm_autorestart_hud				(default: 1)	- Включение вывода HUD сообщений
*		umm_autorestart_time <10-90>	(default: 20)	- Время в секундах для авторестарта (только для режима TIME)
*		umm_autorestart_limit			(default: 3)	- Количество рестартов
*		umm_autorestart_protect <0|1>	(default: 1)	- Защита игроков от повреждений на время рестарта (только для режима TIME)
*		umm_autorestart_voteratio		(default - 0)	- Процент игроков для голосования за рестарт карты
*
...
public plugin_init()
{
	...
	pcv_autorestart				= register_cvar ( "umm_autorestart",			"2" )
	pcv_autorestart_hud			= register_cvar ( "umm_autorestart_hud",		"0" )
	pcv_autorestart_time		= register_cvar ( "umm_autorestart_time",		"20" )
	pcv_autorestart_limit		= register_cvar ( "umm_autorestart_limit",		"1" )
