...
#define PLUGIN "[ZP] SupplyBox SubPlugin: SpawnPoint Maker"
#define VERSION "1.0"
#define AUTHOR "Nguyen Son Tung" // ORIGINAL BY SONTUNG0

new const BOXS_URL[] = "%s/zp_supplybox/%s.cfg"
const MAX_BOXS = 128
...
public plugin_init()
{
	...
	// Cmd
	register_concmd("zp_supplybox_sp_maker", "menu_main")
...
public box_main(id)
{
	...
	format(title, charsmax(title), "[SP Maker] Spawn Point Maker", LANG_PLAYER, "MENU_BOXS_TITLE", g_total_boxs, MAX_BOXS)
...
public box_create(id)
{
	...
	format(title, charsmax(title), "[SP Maker] Spawn Point Add", LANG_PLAYER, "MENU_BOXS_ADD_TITLE", g_total_boxs, MAX_BOXS)
	format(item_name[0], 63, "Add to current location", LANG_PLAYER, "MENU_BOXS_ADD_ITEM_ADD")
	format(item_name[1], 63, "Delete the last spawn point", LANG_PLAYER, "MENU_BOXS_ADD_ITEM_DEL")
...
box_create_add(id)
{
	...
	{
		new message[128]
		format(message, charsmax(message), ">x04[[SP Maker]] >x01Sorry. MAX BOX", LANG_PLAYER, "NOTICE_MAX_BOXS", MAX_BOXS)
	...
	{
		new message[128]
		format(message, charsmax(message), ">x04[SP Maker] >xSorry. MAX DISTANCE", LANG_PLAYER, "NOTICE_BOXS_DIST")
		color_saytext(id, message)
		return;
	}
...
public box_edit(id)
{
	...
	{
		new message[128]
		format(message, charsmax(message), ">x04[SP Maker] >x01 Not Found. You need to Load first", LANG_PLAYER, "NOTICE_BOXS_NOTVALUE")
		color_saytext(id, message)
		box_main(id)
		return;
	}
	...
	format(title, charsmax(title), "[SP Maker] Spawn Point Edit", LANG_PLAYER, "MENU_BOXS_EDIT_TITLE")
	format(item_name[0], 63, "Back", LANG_PLAYER, "MENU_BOXS_EDIT_ITEM_BACK")
	format(item_name[1], 63, "Next", LANG_PLAYER, "MENU_BOXS_EDIT_ITEM_NEXT")
	format(item_name[2], 63, "Edit", LANG_PLAYER, "MENU_BOXS_EDIT_ITEM_EDIT")
	format(item_name[3], 63, "Delete", LANG_PLAYER, "MENU_BOXS_EDIT_ITEM_DEL")
...
box_save()
{
	...
	{
		new message[128]
		format(message, charsmax(message), ">x04[SP Maker] >x01 Not Found. You need to load of create before save", LANG_PLAYER, "NOTICE_BOXS_NOTVALUE")
		color_saytext(0, message)
		return;
	}
	...
	// show notice
	new message[128]
	format(message, charsmax(message), ">x04[SP Maker] >x01 SAVE COMPLETE", LANG_PLAYER, "NOTICE_SAVEBOX_COMPLETE")
	color_saytext(0, message)
}
...
box_load()
{
	...
	{
		new message[128]
		format(message, charsmax(message), ">x04[SP Maker] >x01 File Not Found", LANG_PLAYER, "NOTICE_FILE_EXIT", filepath)
		color_saytext(0, message)
		return;
	}
	...
	{
		new message[128]
		format(message, charsmax(message), ">x04[SP Maker] >x01 Load Complete", LANG_PLAYER, "NOTICE_LOADBOX_COMPLETE", g_total_boxs)
		color_saytext(0, message)
	}
}
...
box_del(all=0)
{
	...
	{
		new message[128]
		format(message, charsmax(message), ">x04[SP Maker] >x01 Not Found. You need to load of create if you want to delete", LANG_PLAYER, "NOTICE_BOXS_NOTVALUE")
		color_saytext(0, message)
		return;
	}
