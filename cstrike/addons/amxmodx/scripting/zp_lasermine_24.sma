...
new g_LENABLE, g_LFMONEY, g_LAMMO, g_LDMG, g_LBEO, g_LTMAX, g_LCOST, g_LHEALTH, g_LMODE, g_LRADIUS
...
public plugin_init()
{
	...
	//CVAR_LMCost = get_cvar_pointer("zp_ltm_cost")
	//g_LME = zp_register_extra_item("Laser Mine", get_pcvar_num(CVAR_LMCost), ZP_TEAM_ANY)
	g_LME = zp_register_extra_item("Laser Mine", LMCost, ZP_TEAM_ANY)
	...
	register_cvar("Shidla", "ZP LaserMines v.2.4 Final", FCVAR_SERVER|FCVAR_SPONLY)
}
...
{
	if( !get_pcvar_num( g_LENABLE ) || zp_is_nemesis_round() || zp_is_survivor_round() || !zp_has_round_started())
	{
...
public ltm_Think( i_Ent )
{
	...
	if( !get_pcvar_num( g_LENABLE ) || zp_is_nemesis_round() || zp_is_survivor_round() || !zp_has_round_started()) return FMRES_IGNORED;
...
public newround(id){
	if( !get_pcvar_num( g_LENABLE ) || zp_is_nemesis_round() || zp_is_survivor_round())
		return PLUGIN_CONTINUE
	pev(id, pev_maxspeed,plspeed[id])
	DeleteTask(id);
	RemoveAllTripmines(id);
