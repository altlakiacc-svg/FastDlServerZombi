...
{
	if(get_pcvar_num(g_CvarAllowDonate)) {
		client_printcolor(0, "!g[ZP] !yДля передачи Кредитов введите !g/donate")
		client_printcolor(0, "")
}
}
...
public donate(id, arg[])
{
	...
	if (!to[0] || !count[0])
	{
		client_printcolor(id, "!g[ZP] !yПример: say /donate <ник> <число>")
		return
	}
	...
	if (ammo <= 0)
	{
		client_printcolor(id, "!g[ZP] !yНеправильное колличество Кредитов!")
		return
	}
	ammo_sender -= ammo
	if (ammo_sender < 0)
	...
	{
		client_printcolor(id, "!g[ZP] !yИгрока !g%s !yнет на сервере!", to)
		return
	}
	...
	show_hudmessage(0, "%s передал %d для %s!", aName, ammo, vName)
